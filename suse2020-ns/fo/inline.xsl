...
<xsl:template name="inline.text">
  ...
  <fo:inline>
   <xsl:if test="$lighter-formatting != 1">
    <xsl:attribute name="border-bottom">&thinline;mm solid &bw-mid-gray;</xsl:attribute>
...
<xsl:template name="inline.italicmonoseq">
  ...
  <fo:inline xsl:use-attribute-sets="monospace.properties italicized"
    font-weight="normal" color="{$textcolor}">
   <xsl:call-template name="inline.text">
    <xsl:with-param name="mode" select="$mode"/>
    <xsl:with-param name="after" select="$after"/>
    <xsl:with-param name="before" select="$before"/>
...
<xsl:template name="inline.boldseq">
  ...
  <fo:inline xsl:use-attribute-sets="serif.bold" color="{$textcolor}">
    <xsl:if test="@dir">
      <xsl:attribute name="direction">
        <xsl:choose>
          <xsl:when test="@dir = 'ltr' or @dir = 'lro'">ltr</xsl:when>
...
<xsl:template name="inline.italicseq">
  ...
  <fo:inline xsl:use-attribute-sets="italicized" color="{$textcolor}">
    <xsl:choose>
...
<xsl:template match="d:command|d:userinput">
  ...
  <xsl:param name="commandcolor">
    <xsl:choose>
      <xsl:when test="self::d:command and ancestor::d:screen and $format.print = 0">
        &suse-darker-green;
      </xsl:when>
    ...
    <svg:svg xmlns:svg="http://www.w3.org/2000/svg" height="100"
      ...
      <svg:g>
        ...
        <svg:rect height="100" width="{$width + 60}" rx="10" ry="10" x="0" y="0"
          fill="{$corporate-blue}" fill-opacity="1" stroke="none"/>
      </svg:g>
      ...
      <svg:text font-family="{$mono-stack}" text-anchor="middle"
        x="{($width div 2) + 25}" y="{$instream-font-size}" fill="{$corporate-blue}"
...
<xsl:template match="d:keycombo">
  ...
  <xsl:for-each select="*">
    <xsl:if test="position()>1">
      <fo:inline space-start="-0.05em" space-end="0" color="{$corporate-blue}">
...
<xsl:template match="d:prompt">
  <xsl:variable name="promptcolor">
    <xsl:choose>
      <xsl:when test="@role = 'root' and $format.print = 0">&suse-dark-pink;</xsl:when>
      <xsl:otherwise>&bw-mid-gray;</xsl:otherwise>
    </xsl:choose>
  </xsl:variable>

  <xsl:call-template name="inline.monoseq">
    <xsl:with-param name="textcolor" select="$promptcolor"/>
  </xsl:call-template>
</xsl:template>
    ...
    <xsl:element name="fo:{$wrapper}" use-attribute-sets="italicized.noreplacement">
      <xsl:choose>
        <xsl:when test="$format.print != 1">
          <xsl:attribute name="background-color">&suse-purple;</xsl:attribute>
          <xsl:attribute name="color">&white;</xsl:attribute>
        </xsl:when>
        <xsl:otherwise>
          <xsl:attribute name="color">&bw-mid-gray;</xsl:attribute>
        </xsl:otherwise>
      </xsl:choose>
