...
<xsl:template name="admon.symbol">
  <xsl:param name="color" select="'&suse-dark-green;'"/>
  ...
  <svg:svg width="36" height="36">
    <svg:g>
      ...
      <xsl:choose>
        ...
        important is orange and caution is red. -->
...
<xsl:template name="admon.symbol.color">
  ...
  <xsl:choose>
    <xsl:when test="$format.print = 1">
      <xsl:text>&bw-darker-gray;</xsl:text>
    </xsl:when>
    <xsl:otherwise>
      <xsl:choose>
        <xsl:when test="local-name($node)='warning' or
          ...
          <xsl:text>&suse-dark-red;</xsl:text>
        </xsl:when>
        <xsl:when test="local-name($node)='tip'">
          <xsl:text>&suse-dark-green;</xsl:text>
        </xsl:when>
        <xsl:when test="local-name($node)='important'">
          <xsl:text>&suse-dark-orange;</xsl:text>
        </xsl:when>
        <xsl:otherwise>
          <!-- It's a note. (Or something undefined.) -->
          <xsl:text>&suse-corporate-blue;</xsl:text>
        </xsl:otherwise>
      </xsl:choose>
    </xsl:otherwise>
  </xsl:choose>
 ...
 <fo:block>
  <xsl:attribute name="color">
   <xsl:choose>
    ...
    <xsl:otherwise>&bw-darker-gray;</xsl:otherwise>
   </xsl:choose>
  </xsl:attribute>
